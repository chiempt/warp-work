...
| **Account** | A connected source of signals, with a reliability tier. One Google account produces three of these — `gmail`, `gcalendar`, `gdrive`. | A person, and **not a login**. The owner's sign-in lives in `auth_providers`. |
| **Person** | A human the owner deals with, reachable through one or more **identities** (email address, Zalo ID, phone number). | A user. There is exactly one user — the owner, who is deliberately *not* in `people`: people are the counterparties in commitments. |
...
| **Work session** | The clock-in. Scopes which contexts agents may touch and for how long, and carries the token budget. Agents do not run outside one. | An **auth session**, which is the browser being signed in. Different table, different lifetime. |
| **Auth provider** | One way the owner can sign in: a provider plus that provider's immutable `subject` for them. Several may exist; the extras are the recovery path. | A **contact identity** (`identities`), which belongs to a person. Or an **account**, which is a data source. |
| **User** | The identity root — an id and nothing else, because a hundred foreign keys point at it. What describes the person lives in the **user profile**. | An **auth provider**, which is how they sign in. |
| **Auth session** | A browser that is currently signed in. Server-side, revocable, stored as a hash of its token. | A **work session**. |
