...
| **Account** | A connected source of signals, with a reliability tier. One Google account produces three of these — `gmail`, `gcalendar`, `gdrive`. | A person, and **not a login**. The owner's sign-in lives in `auth_identities`. |
...
| **Auth identity** | One way the owner can sign in: a provider plus that provider's immutable `subject` for them. Several may exist; the extras are the recovery path. | A **contact identity** (`identities`), which belongs to a person. Or an **account**, which is a data source. |
