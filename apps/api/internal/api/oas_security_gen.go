...
type SecurityHandler interface {
	...
	// An opaque session token. The server stores only its hash, so a database dump is not enough to sign
	// in. `HttpOnly`, so a scripting flaw in the dashboard cannot read it.
...
func (s *Server) securitySessionCookie(ctx context.Context, operationName OperationName, req *http.Request) (context.Context, bool, error) {
	var t SessionCookie
	const parameterName = "warp_session"
	var value string
