...
4. **Autonomy is per `(context, action_type)`.** Never a global flag, never a per-user flag, never an
   env var. Default is `draft`. Levels rise only through `autonomy_evidence`.

## Phase discipline

Build in the order of context doc §9. Do not implement a later phase's behaviour early, even when
...
`docs/api/openapi.yaml` is the source of truth. `oapi-codegen` generates the Echo server interfaces
and the frontend's TypeScript client from it. **Never hand-write either side.** Go and TypeScript
cannot share types by construction, so the spec is the only thing keeping them honest — a handler
that drifts from it must fail to compile.
...
- Enum-like columns are Postgres `text` with a `CHECK` constraint, not native `ENUM` types —
  cheaper to extend as the autonomy ladder and action kinds grow.
