...
4. **The owner can always get back in.** `auth_providers` may never drop to zero rows — there is
...
- Enum-like columns are native Postgres `ENUM` types, declared in the migration of the module that
  owns them. Adding a value needs its own `-- +goose NO TRANSACTION` migration; a value can never be
  removed.
